...
| コマンド | 説明 |
| -------- | ---- |
| `./sync.sh sync` | `開始`〜`完了`/`中断` の時間をTogglにタイムエントリとして登録する |
| `./sync.sh report` | 今日の投稿をプロジェクトごとにまとめて標準出力に書き起こす（Slack貼り付け用） |
| `./sync.sh todos` | 未完了タスクの一覧を表示する |
...
`sync` 実行時に、`完了` で締まっていないタスクが自動的にDBに保存される。`完了` が追加されて再度 `sync` を実行すると、DBから削除される。
